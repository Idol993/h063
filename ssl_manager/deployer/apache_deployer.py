...
@dataclass
class ApacheDeployResult:
    """部署操作结果"""
    success: bool
    failure_type: str = ""
    failure_detail: str = ""
    deployed_target_files: List[str] = field(default_factory=list)
    rollback: Optional[ApacheRollbackResult] = None


class ApacheDeployer:
    def __init__(
        self,
        apachectl_bin: str = "apachectl",
        reload_command: str = "apachectl graceful",
        backup_dir: str = "./backups",
        backup_count: int = 5,
    ):
        self.apachectl_bin = apachectl_bin
        self.reload_command = reload_command
        self.backup_manager = BackupManager(backup_dir, backup_count)

    def deploy(self, domain: str, cert_path: str, key_path: str, target_cert_path: str, target_key_path: str, chain_path: str = "", target_chain_path: str = "") -> ApacheDeployResult:
        targets = [target_cert_path, target_key_path]
        if target_chain_path:
            targets.append(target_chain_path)
        result = ApacheDeployResult(success=False, deployed_target_files=targets)

        logger.info(f"部署 Apache 证书: {domain}")

        try:
            self._backup_existing_certs(domain, target_cert_path, target_key_path, target_chain_path)
            self._copy_certificates(cert_path, key_path, chain_path, target_cert_path, target_key_path, target_chain_path)

            test_ok, test_msg = self._test_config()
            if not test_ok:
                result.failure_type = "config_test_failed"
                result.failure_detail = test_msg
                logger.error(f"Apache 配置测试未通过，回滚中...")
                result.rollback = self._rollback(domain, target_cert_path, target_key_path, target_chain_path)
                return result

            reload_ok, reload_msg = self._reload_apache()
            if not reload_ok:
                result.failure_type = "reload_failed"
                result.failure_detail = reload_msg
                logger.error(f"Apache 热重载失败，回滚中...")
                result.rollback = self._rollback(domain, target_cert_path, target_key_path, target_chain_path)
                return result

            result.success = True
            logger.info(f"Apache 证书部署成功: {domain}")
            return result

        except FileNotFoundError as e:
            result.failure_type = "not_found_cmd"
            result.failure_detail = str(e)
            logger.error(f"apachectl 命令未找到: {e}")
        except Exception as e:
            result.failure_type = "exception"
            result.failure_detail = str(e)
            logger.error(f"Apache 部署异常: {e}")
    ...
    def _backup_existing_certs(self, domain: str, cert_path: str, key_path: str, chain_path: str = ""):
        files_to_backup = []
        ...
        if chain_path and Path(chain_path).exists():
            files_to_backup.append(chain_path)
    ...
    def _copy_certificates(self, src_cert: str, src_key: str, src_chain: str, dst_cert: str, dst_key: str, dst_chain: str):
        import os
        import shutil
        ...
        if src_chain and dst_chain:
            dst_chain_p = Path(dst_chain)
            dst_chain_p.parent.mkdir(parents=True, exist_ok=True)
            shutil.copy2(src_chain, dst_chain)
            try:
    ...
    def _test_config(self):
        ...
        try:
            result = subprocess.run(
                [self.apachectl_bin, "configtest"],
                capture_output=True,
                text=True,
                timeout=30,
            )
            if result.returncode == 0:
                logger.info("Apache 配置测试通过")
                return True, ""
            else:
                detail = result.stderr.strip() or f"apachectl configtest exit code {result.returncode}"
                logger.error(f"Apache 配置测试失败: {detail}")
                return False, detail
        except FileNotFoundError:
            msg = f"apachectl 命令未找到: {self.apachectl_bin}"
            logger.error(f"{msg}，部署失败")
            return False, msg
        except subprocess.TimeoutExpired:
            msg = "apachectl configtest 超时 (30s)"
            logger.error(msg)
            return False, msg
        except Exception as e:
            msg = f"apachectl configtest 异常: {e}"
            logger.error(msg)
            return False, msg
    ...
    def _reload_apache(self):
        ...
        try:
            ...
            msg = f"apachectl reload 超时 (30s)"
            logger.error(msg)
            return False, msg
        except Exception as e:
            msg = f"apachectl reload 异常: {e}"
            logger.error(msg)
            return False, msg

    def _rollback(self, domain: str, cert_path: str, key_path: str, chain_path: str = "") -> ApacheRollbackResult:
        rb = ApacheRollbackResult(success=False)
        backups = self.backup_manager.list_backups(domain)
        cert_name = Path(cert_path).name
        key_name = Path(key_path).name
        chain_name = Path(chain_path).name if chain_path else ""

        target_path_map: Dict[str, str] = {
            cert_name: cert_path,
            key_name: key_path,
        }
        if chain_name and chain_path:
            target_path_map[chain_name] = chain_path

        all_targets = [cert_path, key_path]
        if chain_path:
            all_targets.append(chain_path)
        ...
        if in_backup:
            ...
            rb.missing_files = missing
            rb.success = False
            expected_names = ", ".join(target_path_map.keys())
            rb.detail = f"备份中未找到任何期望的证书文件 (期望: {expected_names})"
    ...
    def get_cert_paths(self, domain: str, base_path: Optional[str] = None) -> tuple:
        if base_path:
            ...
            base = Path("/etc/httpd/certs")

        cert_path = base / domain / "cert.pem"
        key_path = base / domain / "privkey.pem"
        chain_path = base / domain / "chain.pem"
        return str(cert_path), str(key_path), str(chain_path)
